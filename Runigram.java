...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int row = 0; row < image.length; row++) {
			for (int col = 0; col < image[0].length; col++) {
				hFlip [row][col] = image [row][ncol - 1 - col];
			}
		}
		return hFlip;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int row = 0; row < image.length; row++) {
			for (int col = 0; col < image[0].length; col++) {
				vFlip [row][col] = image [image.length - 1 - row][col];
			}
		}
		return vFlip;
	}
	...
	public static Color[][] grayScaled(Color[][] image) {
		...
		for (int rows = 0; rows< image.length; rows++) {
			for (int col = 0; col < image[0].length; col++){
				gray[rows][col] = luminance(image[rows][col]);
			}
		}
		return gray;
	}	
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++){
			for (int j = 0; j < width; j++){
				ans[i][j] = image[i * oldRow / height][j * oldCol / width];
			}
		}
		return ans;
	}
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		...
		Color[][] blended = new Color [rows][col];
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		Color[][] scaledIm = scaled(target, rows, col);

		Runigram.setCanvas(source);

		for (int i = 0; i < n; i++){
			double alpha = i / (double) n;
			Color[][] frame = blend(source, scaledIm, alpha);
